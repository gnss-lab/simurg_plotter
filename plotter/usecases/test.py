...
plotter = PlotManager(nrows=3, ncols=2, height=10)
...
dst_values = np.random.uniform(low=-100, high=100, size=len(timestamps))
dst_data = np.column_stack((timestamps, dst_values))
plotter.plot_gim(1, 0, gd[time], title=f'IRI(2017) {time.strftime("%Y-%m-%d %H:%M:%S")}', time=time)
plotter.plot_gim(1, 1, gd[time2], title=f'IRI(2017) {time2.strftime("%Y-%m-%d %H:%M:%S")}', colorbar=True, time=time2)


plotter.plot_dst(2, 0, dst_data, title = "DST Index", time=times[10], colspan=2)
plotter.save("combined_plot.png")

for i in range(6):
    plotter = PlotManager(nrows=1, ncols=3, height_ratios=[1], height=4, margin=0.2)
    plotter.plot_map2d(0, 2, data[times[i+2]], title="Map2D", colorbar=True, product_type="dtec_2_10", time=times[i+2], save_fig=None, polar=False, subsolar=True, min_lat=50, max_lat=55, min_lon=100, max_lon=110)
