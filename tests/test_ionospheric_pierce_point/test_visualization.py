...
from simurg_plotter.ionospheric_pierce_point.visualization import (
    mercator_plot,
    polar_plot,
)
...
def test_mercator_plot(
    ...
    for (
        ...
        url = (
            BASE_URL
            + f"ipp_mercator_{cutoff}_cutoff.png"
        )
        downloaded_image_path = download_image(
            url,
            f"test_mercator_{cutoff}",
            TEST_IMAGE_PATH,
        )
        filename = (
            TEST_IMAGE_PATH
        ...
        if (
            ...
            mercator_plot(
                get_series,
                filename=filename,
                elevation_cutoff=deg,
            )
        result = plt.imread(
            filename
        )
        downloaded_image = plt.imread(
            downloaded_image_path
        )
        assert (
            np.corrcoef(
                result.flatten(),
                downloaded_image.flatten(),
            )[
                0,
                1,
            ]
...
def test_polar_plot(
    ...
    for (
        ...
        url = (
            BASE_URL
            + f"ipp_polar_{cutoff}_cutoff.png"
        )
        downloaded_image_path = download_image(
            url,
            f"test_polar_{cutoff}",
            TEST_IMAGE_PATH,
        )
        filename = (
            TEST_IMAGE_PATH
